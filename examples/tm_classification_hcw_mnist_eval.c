...
static void get_input_int8_data(const char* image_file, int8_t* input_data, int img_h, int img_w, int input_channel, float* mean, float* scale,
                                float input_scale)
{
    ...
    for (int i = 0; i < img_w * img_h * input_channel; i++)
    {
...
static int tengine_classify(const char* model_file, const char* image_dir, int img_h, int img_w, int input_channel, float* mean, float* scale, int num_thread)
{
    /* set runtime options */
    struct options opt;
    ...
    context_t hcw_context = create_context("HCW", 1);
    int rtt = set_context_device(hcw_context, "HCW_3020", NULL, 0);
    /* create graph, load tengine model xxx.tmfile */
    graph_t graph = create_graph(hcw_context, "tengine", model_file);
    ...
    int img_size = img_h * img_w * input_channel;
    int dims[] = {1, input_channel, img_h, img_w}; // nchw
    ...
    { // cifar10 10 classes
        ...
        if (d)
        {
            ...
            {
                struct stat st;
                //                if (dir->d_type != DT_REG) continue; // å©µåæ£éæ°­å¯¼å§æå¢é¸æ¬æ·."é¸æ»æ·.."
                sprintf(tmp_image_dir, "%s/%s", tmp_sub_dir, dir->d_name);
                printf("Img name: %s\n", tmp_image_dir);
                /* é¾å³°å½éå¦æ¬¢çç´ï¿½ */
                if (0 != stat(tmp_image_dir, &st))
                {
                    printf("[ERROR] in stat(path[ %s ]) \n", tmp_image_dir);
                    return -1;
                }
                if (S_ISDIR(st.st_mode)) continue; /* é©î¼ç¶ */
                count++;
                /* prepare process input data, set the data mem to input tensor */
                get_tensor_quant_param(input_tensor, &input_scale, &input_zero_point, 1);
                get_input_int8_data(tmp_image_dir, input_data, img_h, img_w, input_channel, mean, scale, input_scale);
                if (set_tensor_buffer(input_tensor, input_data, img_size) < 0)
                {
                    fprintf(stderr, "Set input tensor buffer failed\n");
                    return -1;
...
int tm_classification_hcw_eval_main(char* model_file, char* image_dir, int input_img_size, int input_channel)
{
    int num_thread = DEFAULT_THREAD_COUNT;
    float img_hw[2] = {0.f};
    int img_h = input_img_size;
    int img_w = input_img_size;
    ...
    if (tengine_classify(model_file, image_dir, img_h, img_w, input_channel, mean, scale, num_thread) < 0)
        return -1;
