...
graph_t create_hcw_test_graph(const char* test_node_name, int data_type, int layout, int n, int c, int h, int w, common_test test_func, int dims_num = 4)
{
    /* create hcw backend */
    ...
    graph_t graph = create_graph(hcw_context, NULL, NULL);
    if (NULL == graph)
    {
        fprintf(stderr, "get graph failed.\n");
        return NULL;
