...
int create_test_conv_node(graph_t graph, const char* input_name, const char* node_name, int data_type, int layout, int n, int c, int h, int w)
{
    ...
    tensor_t bias_tensor = create_graph_tensor(graph, "bias", TENGINE_DT_INT8);
...
int test_hcw_op_conv_main()
{
    ...
    // set input data
    set_tensor_buffer(input_tensor, input_data, 16 * sizeof(int8_t));

    // set weight data
    set_tensor_buffer(weight_tensor, wb_data, 18 * sizeof(int8_t));

    // set bias data
    set_tensor_buffer(bias_tensor, bias_data, 2 * sizeof(int8_t));
