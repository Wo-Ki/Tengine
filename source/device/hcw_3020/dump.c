...
void hcw_data_dump(const char* filename, int8_t* data, int w, int h, int c)
{
#ifdef OPENDLA_LOG_
    FILE* fp;
...
int print_tensor_data_value_hcw(FILE* file, const struct tensor* tensor, int offset)
    {
    ...
    const char* get_tensor_data_type_string_hcw(int data_type)
    {
    switch (data_type)
    {
    case TENGINE_DT_FP32:
    ...
    void print_tensor_data_to_file_hcw(FILE* file, const struct tensor* tensor)
        {
    ...
    case 5:
    {
        ...
        else
        {
            ...
            {
                const char* type_name = get_tensor_data_type_string_hcw(tensor->data_type);
                ...
                for (int ch = 0; ch < channel; ch++)
                {
                    ...
                    for (int h = 0; h < height; h++)
                    {
                        ...
                        for (int w = 0; w < width; w++)
                        {
                            ...
                            print_tensor_data_value_hcw(file, tensor, offset);
                        }
                        fprintf(file, "\n");
                    }
                    fprintf(file, "\n");
    ...
    case 1:
    {
        ...
        for (int w = 0; w < width; w++)
        {
            print_tensor_data_value_hcw(file, tensor, w);
        }
    ...
    default:
        ...
        char* replace_string_character_hcw(char* src_str, char* dst_str, char* target_char, char* replaced_char)
        {
    char* p;
    char* _out = dst_str;
    char* _str = src_str;
    ...
    return dst_str;
        ...
        void extract_feature_from_tensor_hcw(const char* comment, const char* layer_name, const struct tensor* tensor)
            {
    ...
    replace_string_character_hcw(layer_short_name, layer_legal_name, "/", "-");
    ...
    print_tensor_data_to_file_hcw(file, tensor);
    ...
    file = NULL;
            ...
            void dump_sub_graph_hcw(struct subgraph* sub_graph)
                {
    printf("Sub graph[%d]: {%8s } has %d nodes, %d input tensors, %d output tensors.\n", sub_graph->index, sub_graph->device->name, sub_graph->node_num, sub_graph->input_num, sub_graph->output_num);
    printf("\tSub nodes: [ ");
